...
class UnitTypeAllOfBedConfiguration(BaseModel):
    """
    UnitTypeAllOfBedConfiguration
    """ # noqa: E501
    bed_type: Optional[StrictStr] = Field(default=None, alias="bedType")
    quantity: Optional[Annotated[int, Field(strict=True, ge=1)]] = None
    __properties: ClassVar[List[str]] = ["bedType", "quantity"]

    @field_validator('bed_type')
    def bed_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['KING', 'QUEEN', 'DOUBLE', 'TWIN', 'SINGLE', 'SOFA_BED', 'BUNK']):
            raise ValueError("must be one of enum values ('KING', 'QUEEN', 'DOUBLE', 'TWIN', 'SINGLE', 'SOFA_BED', 'BUNK')")
        return value
