...
class MenuItem(BaseModel):
    ...
    dietary: Optional[List[StrictStr]] = None
    allergens: Optional[List[StrictStr]] = None
    preparation_time: Optional[StrictInt] = Field(default=None, description="Preparation time in minutes", alias="preparationTime")
    image_url: Optional[StrictStr] = Field(default=None, alias="imageUrl")
    metadata: Optional[Model0] = None
    __properties: ClassVar[List[str]] = ["itemId", "name", "description", "category", "price", "available", "dietary", "allergens", "preparationTime", "imageUrl", "metadata"]

    @field_validator('category')
    def category_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['APPETIZER', 'MAIN_COURSE', 'DESSERT', 'BEVERAGE', 'ALCOHOL', 'BREAKFAST', 'SNACK']):
            raise ValueError("must be one of enum values ('APPETIZER', 'MAIN_COURSE', 'DESSERT', 'BEVERAGE', 'ALCOHOL', 'BREAKFAST', 'SNACK')")
        return value

    @field_validator('dietary')
    def dietary_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['VEGETARIAN', 'VEGAN', 'GLUTEN_FREE', 'DAIRY_FREE', 'NUT_FREE', 'HALAL', 'KOSHER']):
                raise ValueError("each list item must be one of ('VEGETARIAN', 'VEGAN', 'GLUTEN_FREE', 'DAIRY_FREE', 'NUT_FREE', 'HALAL', 'KOSHER')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "price": Model0.from_dict(obj["price"]) if obj.get("price") is not None else None,
            "available": obj.get("available") if obj.get("available") is not None else True,
            "dietary": obj.get("dietary"),
            "allergens": obj.get("allergens"),
            "preparationTime": obj.get("preparationTime"),
