...
class TrainingModule(BaseModel):
    ...
    content: Optional[TrainingModuleAllOfContent] = None
    assessment_required: Optional[StrictBool] = Field(default=False, alias="assessmentRequired")
    passing_score: Optional[Annotated[int, Field(le=100, strict=True, ge=0)]] = Field(default=None, description="Minimum score to pass (percentage)", alias="passingScore")
    metadata: Optional[Model0] = None
    __properties: ClassVar[List[str]] = ["moduleId", "title", "description", "category", "duration", "content", "assessmentRequired", "passingScore", "metadata"]

    @field_validator('category')
    def category_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SAFETY', 'CUSTOMER_SERVICE', 'TECHNICAL', 'COMPLIANCE', 'MANAGEMENT']):
            raise ValueError("must be one of enum values ('SAFETY', 'CUSTOMER_SERVICE', 'TECHNICAL', 'COMPLIANCE', 'MANAGEMENT')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "content": TrainingModuleAllOfContent.from_dict(obj["content"]) if obj.get("content") is not None else None,
            "assessmentRequired": obj.get("assessmentRequired") if obj.get("assessmentRequired") is not None else False,
