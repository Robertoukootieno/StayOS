...
class TrainingEnrollment(BaseModel):
    ...
    status: StrictStr
    enrolled_at: Optional[datetime] = Field(default=None, alias="enrolledAt")
    started_at: Optional[datetime] = Field(default=None, alias="startedAt")
    completed_at: Optional[datetime] = Field(default=None, alias="completedAt")
    expires_at: Optional[datetime] = Field(default=None, alias="expiresAt")
    assessment_score: Optional[Annotated[int, Field(le=100, strict=True, ge=0)]] = Field(default=None, alias="assessmentScore")
    attempts: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="Number of assessment attempts")
    certificate_url: Optional[StrictStr] = Field(default=None, alias="certificateUrl")
    __properties: ClassVar[List[str]] = ["enrollmentId", "moduleId", "staffId", "status", "enrolledAt", "startedAt", "completedAt", "expiresAt", "assessmentScore", "attempts", "certificateUrl"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ENROLLED', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'EXPIRED']):
            raise ValueError("must be one of enum values ('ENROLLED', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'EXPIRED')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "assessmentScore": obj.get("assessmentScore"),
