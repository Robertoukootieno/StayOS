...
class CancellationPolicy(BaseModel):
    ...
    CancellationPolicy
    """ # noqa: E501
    refundability: StrictStr
    free_cancellation_hours: Optional[StrictInt] = Field(default=None, description="Hours before check-in for free cancellation", alias="freeCancellationHours")
    penalty_schedule: Optional[List[CancellationPolicyAllOfPenaltySchedule]] = Field(default=None, alias="penaltySchedule")
    __properties: ClassVar[List[str]] = ["refundability", "freeCancellationHours", "penaltySchedule"]

    @field_validator('refundability')
    def refundability_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FULLY_REFUNDABLE', 'PARTIALLY_REFUNDABLE', 'NON_REFUNDABLE']):
            raise ValueError("must be one of enum values ('FULLY_REFUNDABLE', 'PARTIALLY_REFUNDABLE', 'NON_REFUNDABLE')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in penalty_schedule (list)
        _items = []
        if self.penalty_schedule:
            for _item_penalty_schedule in self.penalty_schedule:
                if _item_penalty_schedule:
                    _items.append(_item_penalty_schedule.to_dict())
            _dict['penaltySchedule'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "refundability": obj.get("refundability"),
            "freeCancellationHours": obj.get("freeCancellationHours"),
            "penaltySchedule": [CancellationPolicyAllOfPenaltySchedule.from_dict(_item) for _item in obj["penaltySchedule"]] if obj.get("penaltySchedule") is not None else None
        })
        return _obj
