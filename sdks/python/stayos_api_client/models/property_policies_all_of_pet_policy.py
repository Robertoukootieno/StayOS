...
class PropertyPoliciesAllOfPetPolicy(BaseModel):
    """
    PropertyPoliciesAllOfPetPolicy
    """ # noqa: E501
    pets_allowed: Optional[StrictBool] = Field(default=False, alias="petsAllowed")
    pet_fee: Optional[Model0] = Field(default=None, alias="petFee")
    pet_types: Optional[List[StrictStr]] = Field(default=None, alias="petTypes")
    __properties: ClassVar[List[str]] = ["petsAllowed", "petFee", "petTypes"]

    @field_validator('pet_types')
    def pet_types_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['DOG', 'CAT', 'BIRD', 'OTHER']):
                raise ValueError("each list item must be one of ('DOG', 'CAT', 'BIRD', 'OTHER')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of pet_fee
        if self.pet_fee:
            _dict['petFee'] = self.pet_fee.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "petFee": Model0.from_dict(obj["petFee"]) if obj.get("petFee") is not None else None,
            "petTypes": obj.get("petTypes")
        })
        return _obj
