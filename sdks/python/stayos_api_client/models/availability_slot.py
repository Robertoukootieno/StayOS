...
class AvailabilitySlot(BaseModel):
    ...
    status: StrictStr
    restrictions: Optional[AvailabilitySlotAllOfRestrictions] = None
    allocations: Optional[List[AvailabilitySlotAllOfAllocations]] = Field(default=None, description="Channel-specific allocations")
    rate: Optional[Model0] = Field(default=None, description="Rate for this date (if applicable)")
    notes: Optional[StrictStr] = Field(default=None, description="Internal notes for this date")
    __properties: ClassVar[List[str]] = ["date", "status", "restrictions", "allocations", "rate", "notes"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['AVAILABLE', 'RESERVED', 'BLOCKED', 'SOLD_OUT']):
            raise ValueError("must be one of enum values ('AVAILABLE', 'RESERVED', 'BLOCKED', 'SOLD_OUT')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "restrictions": AvailabilitySlotAllOfRestrictions.from_dict(obj["restrictions"]) if obj.get("restrictions") is not None else None,
            "allocations": [AvailabilitySlotAllOfAllocations.from_dict(_item) for _item in obj["allocations"]] if obj.get("allocations") is not None else None,
            "rate": Model0.from_dict(obj["rate"]) if obj.get("rate") is not None else None,
            "notes": obj.get("notes")
        })
        return _obj
