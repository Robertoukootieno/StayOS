...
class Supplier(BaseModel):
    ...
    bank_details: Optional[SupplierAllOfBankDetails] = Field(default=None, alias="bankDetails")
    rating: Optional[Union[Annotated[float, Field(le=5, strict=True, ge=1)], Annotated[int, Field(le=5, strict=True, ge=1)]]] = Field(default=None, description="Supplier performance rating")
    notes: Optional[StrictStr] = None
    metadata: Optional[Model0] = None
    __properties: ClassVar[List[str]] = ["supplierId", "name", "category", "status", "contactInfo", "address", "paymentTerms", "taxId", "bankDetails", "rating", "notes", "metadata"]

    @field_validator('category')
    def category_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FOOD_BEVERAGE', 'LINEN', 'CLEANING_SUPPLIES', 'MAINTENANCE', 'FURNITURE', 'TECHNOLOGY', 'UTILITIES', 'OTHER']):
            raise ValueError("must be one of enum values ('FOOD_BEVERAGE', 'LINEN', 'CLEANING_SUPPLIES', 'MAINTENANCE', 'FURNITURE', 'TECHNOLOGY', 'UTILITIES', 'OTHER')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "address": Model0.from_dict(obj["address"]) if obj.get("address") is not None else None,
            "paymentTerms": obj.get("paymentTerms"),
            "taxId": obj.get("taxId"),
            "bankDetails": SupplierAllOfBankDetails.from_dict(obj["bankDetails"]) if obj.get("bankDetails") is not None else None,
