...
class PaginatedResponseAllOfPagination(BaseModel):
    """
    PaginatedResponseAllOfPagination
    """ # noqa: E501
    page_size: StrictInt = Field(description="Number of items in current page", alias="pageSize")
    cursor: Optional[StrictStr] = Field(default=None, description="Cursor for next page (null if no more pages)")
    has_more: StrictBool = Field(description="Whether more pages are available", alias="hasMore")
    total_count: Optional[StrictInt] = Field(default=None, description="Total number of items (optional, expensive to compute)", alias="totalCount")
    __properties: ClassVar[List[str]] = ["pageSize", "cursor", "hasMore", "totalCount"]
