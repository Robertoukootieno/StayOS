...
class KeyCard(BaseModel):
    ...
    card_number: StrictStr = Field(description="Physical card number", alias="cardNumber")
    status: StrictStr
    issued_at: Optional[datetime] = Field(default=None, alias="issuedAt")
    expires_at: Optional[datetime] = Field(default=None, description="Card expiration", alias="expiresAt")
    access_level: Optional[StrictStr] = Field(default='ROOM_ONLY', alias="accessLevel")
    __properties: ClassVar[List[str]] = ["cardId", "cardNumber", "status", "issuedAt", "expiresAt", "accessLevel"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACTIVE', 'DEACTIVATED', 'LOST', 'EXPIRED']):
            raise ValueError("must be one of enum values ('ACTIVE', 'DEACTIVATED', 'LOST', 'EXPIRED')")
        return value
    ...
    def access_level_validate_enum(cls, value):
        ...
        if value not in set(['ROOM_ONLY', 'ROOM_AND_AMENITIES', 'FULL_ACCESS']):
            raise ValueError("must be one of enum values ('ROOM_ONLY', 'ROOM_AND_AMENITIES', 'FULL_ACCESS')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "accessLevel": obj.get("accessLevel") if obj.get("accessLevel") is not None else 'ROOM_ONLY'
        })
        return _obj
