...
class PropertyCertification(BaseModel):
    """
    PropertyCertification
    """ # noqa: E501
    certification_id: UUID = Field(alias="certificationId")
    ...
    __properties: ClassVar[List[str]] = ["certificationId", "propertyId", "type", "name", "certificateNumber", "issuingAuthority", "issuedDate", "expiryDate", "status", "documentUrl", "notes", "metadata"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['BUSINESS_LICENSE', 'HEALTH_PERMIT', 'FIRE_SAFETY', 'LIQUOR_LICENSE', 'TOURISM_CERTIFICATION', 'ENVIRONMENTAL', 'ACCESSIBILITY', 'OTHER']):
            raise ValueError("must be one of enum values ('BUSINESS_LICENSE', 'HEALTH_PERMIT', 'FIRE_SAFETY', 'LIQUOR_LICENSE', 'TOURISM_CERTIFICATION', 'ENVIRONMENTAL', 'ACCESSIBILITY', 'OTHER')")
        return value
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['ACTIVE', 'EXPIRED', 'SUSPENDED', 'REVOKED']):
            raise ValueError("must be one of enum values ('ACTIVE', 'EXPIRED', 'SUSPENDED', 'REVOKED')")
        return value
