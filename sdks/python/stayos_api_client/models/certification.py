...
class Certification(BaseModel):
    ...
    Certification
    """ # noqa: E501
    certification_type: Optional[StrictStr] = Field(default=None, alias="certificationType")
    certificate_number: Optional[StrictStr] = Field(default=None, alias="certificateNumber")
    issued_by: StrictStr = Field(description="Issuing authority", alias="issuedBy")
    issued_date: Optional[date] = Field(default=None, alias="issuedDate")
    expiry_date: date = Field(alias="expiryDate")
    document_url: Optional[StrictStr] = Field(default=None, description="URL to certificate document", alias="documentUrl")
    __properties: ClassVar[List[str]] = ["certificationType", "certificateNumber", "issuedBy", "issuedDate", "expiryDate", "documentUrl"]

    @field_validator('certification_type')
    def certification_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['BUSINESS_LICENSE', 'HEALTH_PERMIT', 'FIRE_SAFETY', 'ISO_CERTIFICATION', 'SUSTAINABILITY', 'ACCESSIBILITY']):
            raise ValueError("must be one of enum values ('BUSINESS_LICENSE', 'HEALTH_PERMIT', 'FIRE_SAFETY', 'ISO_CERTIFICATION', 'SUSTAINABILITY', 'ACCESSIBILITY')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "certificationType": obj.get("certificationType"),
