...
module OpenapiClient
  class ApplicabilityRulesAllOfAdvanceBookingWindow < ApiModelBase
    # Minimum days before check-in
    attr_accessor :minimum_days
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ApplicabilityRulesAllOfAdvanceBookingWindow` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ApplicabilityRulesAllOfAdvanceBookingWindow`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'minimum_days')
        self.minimum_days = attributes[:'minimum_days']
      end

      if attributes.key?(:'maximum_days')
        self.maximum_days = attributes[:'maximum_days']
      end
    end
    ...
    def hash
      [minimum_days, maximum_days].hash
    end
