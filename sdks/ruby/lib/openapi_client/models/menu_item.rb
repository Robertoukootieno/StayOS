  ...
  class MenuItem < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::MenuItem` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::MenuItem`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'dietary')
        if (value = attributes[:'dietary']).is_a?(Array)
          self.dietary = value
        end
      end

      if attributes.key?(:'allergens')
        if (value = attributes[:'allergens']).is_a?(Array)
    ...
    def valid?
      ...
      category_validator = EnumAttributeValidator.new('String', ["APPETIZER", "MAIN_COURSE", "DESSERT", "BEVERAGE", "ALCOHOL", "BREAKFAST", "SNACK"])
    ...
    def category=(category)
      validator = EnumAttributeValidator.new('String', ["APPETIZER", "MAIN_COURSE", "DESSERT", "BEVERAGE", "ALCOHOL", "BREAKFAST", "SNACK"])
    ...
    def hash
      [item_id, name, description, category, price, available, dietary, allergens, preparation_time, image_url, metadata].hash
    end
