...
module OpenapiClient
  class GetNPSMetrics200Response < ApiModelBase
    ...
    attr_accessor :detractors

    attr_accessor :total_responses
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GetNPSMetrics200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GetNPSMetrics200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'promoters')
        self.promoters = attributes[:'promoters']
      end

      if attributes.key?(:'passives')
        self.passives = attributes[:'passives']
      end

      if attributes.key?(:'detractors')
        self.detractors = attributes[:'detractors']
      end

      if attributes.key?(:'total_responses')
        self.total_responses = attributes[:'total_responses']
      end
    ...
    def hash
      [nps_score, promoters, passives, detractors, total_responses, trend].hash
    end
