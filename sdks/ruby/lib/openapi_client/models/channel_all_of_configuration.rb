...
module OpenapiClient
  class ChannelAllOfConfiguration < ApiModelBase
    attr_accessor :auto_accept_bookings

    attr_accessor :sync_inventory

    attr_accessor :sync_rates

    attr_accessor :sync_restrictions
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ChannelAllOfConfiguration` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ChannelAllOfConfiguration`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'auto_accept_bookings')
        self.auto_accept_bookings = attributes[:'auto_accept_bookings']
      else
        self.auto_accept_bookings = false
      end

      if attributes.key?(:'sync_inventory')
        self.sync_inventory = attributes[:'sync_inventory']
      else
        self.sync_inventory = true
      end

      if attributes.key?(:'sync_rates')
        self.sync_rates = attributes[:'sync_rates']
      else
        self.sync_rates = true
      end

      if attributes.key?(:'sync_restrictions')
        self.sync_restrictions = attributes[:'sync_restrictions']
      else
        self.sync_restrictions = true
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          auto_accept_bookings == o.auto_accept_bookings &&
          sync_inventory == o.sync_inventory &&
          sync_rates == o.sync_rates &&
          sync_restrictions == o.sync_restrictions
    end
    ...
    def hash
      [auto_accept_bookings, sync_inventory, sync_rates, sync_restrictions].hash
    end
