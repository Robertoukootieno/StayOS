...
module OpenapiClient
  class CompleteMaintenanceRequestRequest < ApiModelBase
    attr_accessor :notes

    attr_accessor :parts_used
    ...
    def self.openapi_types
      {
        :'notes' => :'String',
        :'parts_used' => :'Array<CompleteMaintenanceRequestRequestPartsUsedInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CompleteMaintenanceRequestRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CompleteMaintenanceRequestRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'parts_used')
        if (value = attributes[:'parts_used']).is_a?(Array)
          self.parts_used = value
        end
      end
    end
