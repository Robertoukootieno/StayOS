...
module OpenapiClient
  class SyncChannelRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SyncChannelRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SyncChannelRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      sync_type_validator = EnumAttributeValidator.new('String', ["FULL", "INCREMENTAL", "RATES_ONLY", "AVAILABILITY_ONLY"])
      return false unless sync_type_validator.valid?(@sync_type)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] sync_type Object to be assigned
    def sync_type=(sync_type)
      validator = EnumAttributeValidator.new('String', ["FULL", "INCREMENTAL", "RATES_ONLY", "AVAILABILITY_ONLY"])
      unless validator.valid?(sync_type)
        fail ArgumentError, "invalid value for \"sync_type\", must be one of #{validator.allowable_values}."
      end
      @sync_type = sync_type
    end
