...
module OpenapiClient
  class ComplianceReport < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ComplianceReport` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ComplianceReport`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'submitted_to')
        self.submitted_to = attributes[:'submitted_to']
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["GUEST_REGISTRY", "TAX_REPORT", "OCCUPANCY_REPORT", "REVENUE_REPORT", "GDPR_EXPORT"])
      return false unless type_validator.valid?(@type)
      return false if @property_id.nil?
      return false if @start_date.nil?
      return false if @end_date.nil?
      status_validator = EnumAttributeValidator.new('String', ["GENERATING", "COMPLETED", "FAILED", "SUBMITTED"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["GUEST_REGISTRY", "TAX_REPORT", "OCCUPANCY_REPORT", "REVENUE_REPORT", "GDPR_EXPORT"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["GENERATING", "COMPLETED", "FAILED", "SUBMITTED"])
    ...
    def hash
      [report_id, type, property_id, start_date, end_date, status, file_url, submitted_to, submitted_at, generated_at, metadata].hash
    end
