...
module OpenapiClient
  class SearchAvailability200ResponseResultsInner < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'rate_strategy' => :'RateStrategy',
        :'total_price' => :'Money',
        :'price_breakdown' => :'Array<SearchAvailability200ResponseResultsInnerPriceBreakdownInner>',
        :'availability' => :'String',
        :'cancellation_policy' => :'CancellationPolicy'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SearchAvailability200ResponseResultsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SearchAvailability200ResponseResultsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'price_breakdown')
        if (value = attributes[:'price_breakdown']).is_a?(Array)
          self.price_breakdown = value
        end
      end
    ...
    def valid?
      ...
      availability_validator = EnumAttributeValidator.new('String', ["AVAILABLE", "LIMITED", "LAST_ROOM"])
    ...
    def availability=(availability)
      validator = EnumAttributeValidator.new('String', ["AVAILABLE", "LIMITED", "LAST_ROOM"])
    ...
    def hash
      [inventory_unit_id, unit_type, property, rate_strategy, total_price, price_breakdown, availability, cancellation_policy].hash
    end
