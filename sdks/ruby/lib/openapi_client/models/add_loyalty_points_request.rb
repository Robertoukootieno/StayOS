...
module OpenapiClient
  class AddLoyaltyPointsRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::AddLoyaltyPointsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::AddLoyaltyPointsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @points < 1
        invalid_properties.push('invalid value for "points", must be greater than or equal to 1.')
      end
    ...
    def points=(points)
      ...
      if points < 1
        fail ArgumentError, 'invalid value for "points", must be greater than or equal to 1.'
      end
    ...
    def hash
      [points, reason, reference_id].hash
    end
