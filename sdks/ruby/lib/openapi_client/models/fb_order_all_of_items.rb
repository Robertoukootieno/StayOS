...
module OpenapiClient
  class FBOrderAllOfItems < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::FBOrderAllOfItems` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::FBOrderAllOfItems`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["PENDING", "PREPARING", "READY", "SERVED"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["PENDING", "PREPARING", "READY", "SERVED"])
    ...
    def hash
      [menu_item_id, name, quantity, unit_price, special_instructions, status].hash
    end
