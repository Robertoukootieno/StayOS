...
module OpenapiClient
  class PricingModelAllOfLengthOfStayDiscounts < ApiModelBase
    attr_accessor :minimum_nights
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PricingModelAllOfLengthOfStayDiscounts` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PricingModelAllOfLengthOfStayDiscounts`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'minimum_nights')
        self.minimum_nights = attributes[:'minimum_nights']
      end
    ...
    def valid?
      ...
      discount_type_validator = EnumAttributeValidator.new('String', ["PERCENTAGE", "FIXED_AMOUNT", "FREE_NIGHTS"])
    ...
    def discount_type=(discount_type)
      validator = EnumAttributeValidator.new('String', ["PERCENTAGE", "FIXED_AMOUNT", "FREE_NIGHTS"])
    ...
    def hash
      [minimum_nights, discount_type, discount_value, description].hash
    end
