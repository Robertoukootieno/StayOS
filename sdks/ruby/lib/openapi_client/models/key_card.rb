  ...
  class KeyCard < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::KeyCard` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::KeyCard`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["ACTIVE", "DEACTIVATED", "LOST", "EXPIRED"])
      return false unless status_validator.valid?(@status)
      access_level_validator = EnumAttributeValidator.new('String', ["ROOM_ONLY", "ROOM_AND_AMENITIES", "FULL_ACCESS"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["ACTIVE", "DEACTIVATED", "LOST", "EXPIRED"])
    ...
    def access_level=(access_level)
      validator = EnumAttributeValidator.new('String', ["ROOM_ONLY", "ROOM_AND_AMENITIES", "FULL_ACCESS"])
    ...
    def hash
      [card_id, card_number, status, issued_at, expires_at, access_level].hash
    end
