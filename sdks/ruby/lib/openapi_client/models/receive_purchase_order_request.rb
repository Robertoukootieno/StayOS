...
module OpenapiClient
  class ReceivePurchaseOrderRequest < ApiModelBase
    ...
    def self.openapi_types
      {
        :'received_items' => :'Array<ReceivePurchaseOrderRequestReceivedItemsInner>',
        :'received_by' => :'String',
        :'notes' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ReceivePurchaseOrderRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ReceivePurchaseOrderRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'received_items')
        if (value = attributes[:'received_items']).is_a?(Array)
          self.received_items = value
        end
      end
    ...
    def hash
      [received_items, received_by, notes].hash
    end
