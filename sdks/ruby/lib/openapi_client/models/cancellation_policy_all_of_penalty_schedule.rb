...
module OpenapiClient
  class CancellationPolicyAllOfPenaltySchedule < ApiModelBase
    attr_accessor :hours_before_check_in

    attr_accessor :penalty_type

    attr_accessor :penalty_value
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CancellationPolicyAllOfPenaltySchedule` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CancellationPolicyAllOfPenaltySchedule`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'hours_before_check_in')
        self.hours_before_check_in = attributes[:'hours_before_check_in']
      end
      ...
      if attributes.key?(:'penalty_value')
        self.penalty_value = attributes[:'penalty_value']
      end
    ...
    def valid?
      ...
      penalty_type_validator = EnumAttributeValidator.new('String', ["PERCENTAGE", "FIXED_AMOUNT", "NIGHTS"])
      return false unless penalty_type_validator.valid?(@penalty_type)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] penalty_type Object to be assigned
    def penalty_type=(penalty_type)
      validator = EnumAttributeValidator.new('String', ["PERCENTAGE", "FIXED_AMOUNT", "NIGHTS"])
      unless validator.valid?(penalty_type)
        fail ArgumentError, "invalid value for \"penalty_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          hours_before_check_in == o.hours_before_check_in &&
    ...
    def hash
      [hours_before_check_in, penalty_type, penalty_value, description].hash
    end
