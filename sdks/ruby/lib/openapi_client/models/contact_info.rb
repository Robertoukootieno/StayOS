...
module OpenapiClient
  class ContactInfo < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ContactInfo`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^\+[1-9]\d{1,14}$/)
      if !@alternate_phone.nil? && @alternate_phone !~ pattern
        invalid_properties.push("invalid value for \"alternate_phone\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if !@alternate_phone.nil? && @alternate_phone !~ Regexp.new(/^\+[1-9]\d{1,14}$/)
      preferred_contact_method_validator = EnumAttributeValidator.new('String', ["EMAIL", "PHONE", "SMS", "WHATSAPP"])
      return false unless preferred_contact_method_validator.valid?(@preferred_contact_method)
      true
    end
    ...
    def alternate_phone=(alternate_phone)
      if alternate_phone.nil?
        fail ArgumentError, 'alternate_phone cannot be nil'
      end

      pattern = Regexp.new(/^\+[1-9]\d{1,14}$/)
      if alternate_phone !~ pattern
        fail ArgumentError, "invalid value for \"alternate_phone\", must conform to the pattern #{pattern}."
      end
    ...
    # @param [Object] preferred_contact_method Object to be assigned
    def preferred_contact_method=(preferred_contact_method)
      validator = EnumAttributeValidator.new('String', ["EMAIL", "PHONE", "SMS", "WHATSAPP"])
      unless validator.valid?(preferred_contact_method)
        fail ArgumentError, "invalid value for \"preferred_contact_method\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [email, phone, alternate_phone, preferred_contact_method].hash
    end
