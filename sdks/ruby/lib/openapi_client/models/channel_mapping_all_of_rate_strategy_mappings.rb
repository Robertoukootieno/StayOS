...
module OpenapiClient
  class ChannelMappingAllOfRateStrategyMappings < ApiModelBase
    attr_accessor :internal_rate_strategy_id

    attr_accessor :channel_rate_plan_id

    attr_accessor :channel_rate_plan_name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ChannelMappingAllOfRateStrategyMappings` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ChannelMappingAllOfRateStrategyMappings`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'internal_rate_strategy_id')
        self.internal_rate_strategy_id = attributes[:'internal_rate_strategy_id']
      end

      if attributes.key?(:'channel_rate_plan_id')
        self.channel_rate_plan_id = attributes[:'channel_rate_plan_id']
      end

      if attributes.key?(:'channel_rate_plan_name')
        self.channel_rate_plan_name = attributes[:'channel_rate_plan_name']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          internal_rate_strategy_id == o.internal_rate_strategy_id &&
          channel_rate_plan_id == o.channel_rate_plan_id &&
          channel_rate_plan_name == o.channel_rate_plan_name
    end
    ...
    def hash
      [internal_rate_strategy_id, channel_rate_plan_id, channel_rate_plan_name].hash
    end
