...
module OpenapiClient
  class UnitTypeAllOfBedConfiguration < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UnitTypeAllOfBedConfiguration` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UnitTypeAllOfBedConfiguration`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      bed_type_validator = EnumAttributeValidator.new('String', ["KING", "QUEEN", "DOUBLE", "TWIN", "SINGLE", "SOFA_BED", "BUNK"])
      return false unless bed_type_validator.valid?(@bed_type)
      return false if !@quantity.nil? && @quantity < 1
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] bed_type Object to be assigned
    def bed_type=(bed_type)
      validator = EnumAttributeValidator.new('String', ["KING", "QUEEN", "DOUBLE", "TWIN", "SINGLE", "SOFA_BED", "BUNK"])
      unless validator.valid?(bed_type)
        fail ArgumentError, "invalid value for \"bed_type\", must be one of #{validator.allowable_values}."
      end
      @bed_type = bed_type
    end
