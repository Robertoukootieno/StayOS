...
module OpenapiClient
  class CreateInventoryBlockRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CreateInventoryBlockRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CreateInventoryBlockRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      reason_validator = EnumAttributeValidator.new('String', ["MAINTENANCE", "GROUP_BLOCK", "OWNER_USE", "OTHER"])
    ...
    def reason=(reason)
      validator = EnumAttributeValidator.new('String', ["MAINTENANCE", "GROUP_BLOCK", "OWNER_USE", "OTHER"])
    ...
    def hash
      [property_id, unit_type_id, start_date, end_date, quantity, reason, notes].hash
    end
