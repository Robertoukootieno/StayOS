...
module OpenapiClient
  class PricingModelAllOfSeasonalAdjustments < ApiModelBase
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PricingModelAllOfSeasonalAdjustments` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PricingModelAllOfSeasonalAdjustments`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      adjustment_type_validator = EnumAttributeValidator.new('String', ["PERCENTAGE", "FIXED_AMOUNT"])
    ...
    def hash
      [name, start_date, end_date, adjustment_type, adjustment_value, description].hash
    end
