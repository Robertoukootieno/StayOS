...
module OpenapiClient
  class PropertyPoliciesAllOfPetPolicy < ApiModelBase
    attr_accessor :pets_allowed

    attr_accessor :pet_fee
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PropertyPoliciesAllOfPetPolicy` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PropertyPoliciesAllOfPetPolicy`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pet_fee')
        self.pet_fee = attributes[:'pet_fee']
      end

      if attributes.key?(:'pet_types')
        if (value = attributes[:'pet_types']).is_a?(Array)
          self.pet_types = value
        end
      end
    end
    ...
    def hash
      [pets_allowed, pet_fee, pet_types].hash
    end
